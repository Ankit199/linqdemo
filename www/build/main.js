...
var RequiredinfoPage = /** @class */ (function () {
    function RequiredinfoPage(http, navCtrl, navParams) {
        ...
        this.getearlyriser = function (db, type) {
            var Groupcircle = [];
            for (var key in db) {
                if (db.hasOwnProperty(key)) {
                    var info = { user: key, totalmsg: db[key].length };
                    Groupcircle.push(info);
                }
            }
            if (type === 'e') {
                var earlyrisername = new __WEBPACK_IMPORTED_MODULE_1_linqts__["a" /* List */](Groupcircle).OrderByDescending(function (x) { return x.totalmsg; }).First();
                _this.info.earlyriser = earlyrisername.user;
                console.table(earlyrisername.user);
            }
            else if (type === 'l') {
                var latesleer = new __WEBPACK_IMPORTED_MODULE_1_linqts__["a" /* List */](Groupcircle).OrderByDescending(function (x) { return x.totalmsg; }).First();
                _this.info.lateriser = latesleer.user;
    ...
    RequiredinfoPage.prototype.getLocalData = function () {
        ...
        this.http.get('../../assets/chat/chat.txt').map(function (res) { return res.text(); }).subscribe(function (data) {
            ...
            /* Filter Record between Two hour BEGIN  */
            _this.datewisedata = _this.datewisedata.filter(function (x) { return x.message !== undefined; });
            _this.datewisedata.forEach(function (el) {
                var dnew = el.date + ',' + el.time;
                var hh = new Date(dnew);
                el.hour = hh.getHours();
            });
            /** early Riser  */
            var earlyriser = _this.datewisedata.filter(function (x) { return x.hour >= "5" && x.hour <= "8"; });
            var msgflowearly = new __WEBPACK_IMPORTED_MODULE_1_linqts__["a" /* List */](earlyriser).GroupBy(function (y) { return y.user; }, function (x) { return x.message; });
            _this.getearlyriser(msgflowearly, 'e');
            /** Early riser END  */
            /** Late Sleepr */
            var latesleer = _this.datewisedata.filter(function (x) { return x.hour >= "0" && x.hour <= "4"; });
            var msgflowlate = new __WEBPACK_IMPORTED_MODULE_1_linqts__["a" /* List */](latesleer).GroupBy(function (y) { return y.user; }, function (x) { return x.message; });
            _this.getearlyriser(msgflowlate, 'l');
            /** Late Sleepr */
            /** Filter records between two hour END */
